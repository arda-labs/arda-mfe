...
export function customerReport(params: { q?: string; customer_type?: string; status?: string } = {}) {
  const q = buildSearchParams({
    q: params.q,
  ...
  const suffix = q.size ? `?${q.toString()}` : ""
  return getCanonicalList<CustomerReportRow>(`/api/crm/reports/customers${suffix}`)
}
...
export function getCrmProject(id: string) {
  return getCanonical<CrmProjectDetail>(`/api/crm/projects/${encodeURIComponent(id)}`)
}

export function updateCrmProject(id: string, body: Partial<CrmProject>) {
  return putCanonical<CrmProject>(`/api/crm/projects/${encodeURIComponent(id)}`, body)
}

export function addCrmProjectMember(projectId: string, body: { user_id: string; role_code?: string }) {

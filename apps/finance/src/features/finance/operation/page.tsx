...
export function FinanceTransactionSearchPage() {
  ...
  const search = useCallback(async (values: FinanceTransactionSearchParams) => {
    setLoading(true)
    try {
      setResult(await financeOperationApi.searchTransactions(values))
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const onSubmit = form.handleSubmit((values) => setParams(values))
  ...
  return (
    ...
    >
      ...
      {loading ? <LoadingBlock /> : <CaseTable cases={result?.items ?? []} />}
    </FinanceOperationFrame>
  )
}
...
function FinanceOperationPage({ operation }: { operation: FinanceOperation }) {
  ...
  const loadCases = useCallback(async () => {
    setLoading(true)
    try {
      setResult(await financeOperationApi.listCases(operation, view))
    } finally {
      setLoading(false)
    }
  }, [operation, view])
  ...
  const cases = useMemo(() => result?.items ?? [], [result?.items])
              ...
              <TableCell className="text-right">
                {onSelect ? (
                  ...
                  <a
                    ...
                    href={`${item.operation === "incoming" ? "/workbench/incoming-transactions" : "/workbench/outgoing-transactions"}?caseCode=${encodeURIComponent(item.caseCode)}&mode=view`}
                  >
                    <Eye className="size-4" />
                    Mở
                  </a>

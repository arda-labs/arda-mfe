...
export function TrialBalancePage() {
  ...
  const totalDebit = entries
    .filter(
      (entry) =>
        entry.account.type === "ASSET" || entry.account.type === "EXPENSE"
    )
    .reduce(
      (sum, entry) => sum + Number.parseFloat(entry.balance?.balance || "0"),
      0
    )
  const totalCredit = entries
    .filter((entry) =>
      ["LIABILITY", "EQUITY", "INCOME"].includes(entry.account.type)
    )
    .reduce(
      (sum, entry) => sum + Number.parseFloat(entry.balance?.balance || "0"),
      0

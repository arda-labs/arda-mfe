...
import { adminApi, type Tenant, type TenantMember } from "@/features/iam"
...
export function TenantsPage() {
  ...
  const loadTenants = useCallback(async () => {
    setLoading(true)
    try {
      const nextTenants = await adminApi.listTenants()
      setTenants(nextTenants)
      const entries = await Promise.all(
        nextTenants.map(async (tenant) => [
          tenant.id,
          await adminApi.listTenantMembers(tenant.id),
        ] as const)
      )
  ...
  const addMember = async (tenantId: string) => {
    ...
    try {
      await adminApi.addTenantMember(tenantId, userId)
  ...
  const removeMember = async (tenantId: string, userId: string) => {
    try {
      await adminApi.removeTenantMember(tenantId, userId)
  ...
  const createTenant = async () => {
    ...
    setSaving(true)
    try {
      await adminApi.createTenant({ code: normalizedCode, name: normalizedName })
      setCode("")
      setName("")

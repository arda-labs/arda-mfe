import { useMemo, useState } from "react"
import type { ColumnDef } from "@tanstack/react-table"
import { ListTableToolbar } from "@workspace/list-page/list-table-toolbar"
import { textSearchMeta } from "@workspace/list-page/column-filters"
import { Button } from "@workspace/ui/components/button"
import { Pencil } from "lucide-react"
import { useI18n } from "@workspace/i18n"
import { DataTableColumnHeader } from "@workspace/ui/components/data-table/data-table-column-header"
import { useAuthStore } from "@workspace/auth"
import type { WorkflowRoleMembership } from "../../api"
import { workflowApi } from "../../api"
import { RoleMembershipDialog } from "../../shared/admin-ui"
import {
  byString,
  statusFilterOptions,
  useTabList,
  WorkflowStatusCell,
  type CatalogSelectOption,
  type ProcessRolesTab,
} from "./tab-list"

function amountRange(min?: number, max?: number) {
  if (min == null && max == null) return "-"
  return `${min ?? 0} - ${max ?? "..."}`
}

export function useMembershipTab(
  roleCodeOptions: CatalogSelectOption[]
): ProcessRolesTab {
  const { t, formatDate } = useI18n()
  const tenantId = useAuthStore((state) => state.user?.tenantId ?? "")
  const [editing, setEditing] = useState<WorkflowRoleMembership | null>(null)
  const [createOpen, setCreateOpen] = useState(false)

  const columns = useMemo<ColumnDef<WorkflowRoleMembership>[]>(
    () => [
      {
        id: "mem_role",
        accessorKey: "roleCode",
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            label={t("workflow.process_roles.membership_col_role")}
          />
        ),
        enableColumnFilter: true,
        meta: textSearchMeta(
          t("workflow.process_roles.membership_col_role"),
          t("workflow.process_roles.search_role_placeholder")
        ),
        cell: ({ row }) => (
          <span className="font-mono text-sm">{row.original.roleCode}</span>
        ),
      },
      {
        id: "mem_principal",
        accessorKey: "principalId",
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            label={t("workflow.process_roles.membership_col_principal")}
          />
        ),
        cell: ({ row }) =>
          `${row.original.principalType}:${row.original.principalId}`,
      },
      {
        id: "mem_scope",
        accessorKey: "orgId",
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            label={t("workflow.process_roles.membership_col_scope")}
          />
        ),
        enableSorting: false,
        cell: ({ row }) => (
          <span className="text-xs text-muted-foreground">
            {[row.original.orgId, row.original.branchId, row.original.productCode]
              .filter(Boolean)
              .join(" / ") || t("workflow.process_roles.scope_all")}
          </span>
        ),
      },
      {
        id: "mem_limits",
        accessorKey: "minAmount",
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            label={t("workflow.process_roles.membership_col_limits")}
          />
        ),
        cell: ({ row }) =>
          amountRange(row.original.minAmount, row.original.maxAmount),
      },
      {
        id: "mem_effective",
        accessorKey: "effectiveFrom",
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            label={t("workflow.process_roles.membership_col_effective")}
          />
        ),
        cell: ({ row }) => (
          <span className="text-xs">
            {row.original.effectiveFrom
              ? formatDate(row.original.effectiveFrom, { dateStyle: "short" })
              : "-"}
            {" - "}
            {row.original.effectiveTo
              ? formatDate(row.original.effectiveTo, { dateStyle: "short" })
              : "..."}
          </span>
        ),
      },
      {
        id: "mem_status",
        accessorKey: "status",
        header: ({ column }) => (
          <DataTableColumnHeader
            column={column}
            label={t("workflow.process_roles.col_status")}
          />
        ),
        enableColumnFilter: true,
        meta: {
          label: t("workflow.process_roles.col_status"),
          variant: "multiSelect",
          options: statusFilterOptions(t),
        },
        cell: ({ row }) => <WorkflowStatusCell status={row.original.status} />,
      },
      {
        id: "mem_actions",
        header: () => (
          <div className="text-right">{t("workflow.common_col_actions")}</div>
        ),
        enableSorting: false,
        enableHiding: false,
        cell: ({ row }) => (
          <div className="flex justify-end">
            <Button
              variant="ghost"
              size="icon"
              className="size-7 text-muted-foreground"
              onClick={() => setEditing(row.original)}
              title={t("workflow.edit")}
            >
              <Pencil className="size-3.5" />
            </Button>
          </div>
        ),
      },
    ],
    [formatDate, t]
  )

  const { query, table, total } = useTabList<WorkflowRoleMembership>({
    queryKey: ["workflow", "role-memberships", "all", tenantId],
    queryFn: () => workflowApi.listRoleMemberships(tenantId),
    enabled: Boolean(tenantId),
    columns,
    textFilterId: "mem_role",
    textFields: (item) => [item.roleCode, item.principalId],
    statusFilterId: "mem_status",
    sortFields: {
      mem_role: byString((item) => item.roleCode),
      mem_principal: byString((item) => item.principalId),
      mem_limits: byString((item) => String(item.minAmount ?? 0)),
      mem_effective: byString((item) => item.effectiveFrom ?? ""),
      mem_status: byString((item) => item.status),
    },
  })

  const onSaved = () => void query.refetch()
  const dialogProps = { roleOptions: roleCodeOptions, onSaved }

  return {
    key: "membership",
    table,
    total,
    criticalPending: Boolean(tenantId) && query.isPending,
    criticalError: tenantId
      ? query.error
      : new Error(t("workflow.process_roles.tenant_missing")),
    onRetry: () => void query.refetch(),
    fetching: query.isFetching,
    countLabel: t("workflow.process_roles.count", { count: total }),
    toolbar: (
      <ListTableToolbar
        table={table}
        onCreate={() => setCreateOpen(true)}
        createLabel={t("workflow.process_roles.create_membership")}
        exportFilename={t("workflow.process_roles.tab_membership")}
        sheetName={t("workflow.process_roles.tab_membership")}
        totalRowsCount={total}
      />
    ),
    dialogs: (
      <>
        {createOpen ? (
          <RoleMembershipDialog
            open
            onOpenChange={setCreateOpen}
            {...dialogProps}
          />
        ) : null}
        {editing ? (
          <RoleMembershipDialog
            item={editing}
            open
            onOpenChange={(open) => !open && setEditing(null)}
            {...dialogProps}
          />
        ) : null}
      </>
    ),
  }
}

...
export function DpmProductRequestTaskForm({
  ...
  function submit(action: WorkflowTaskAction) {
    ...
    if (requiresComment(task, action) && !trimmed) {
      setError(t("deposit.product_task_form.comment_required"))
      return
    }
    setError("")
    const dataVersion =
    ...
    switch (action) {
      case TASK_ACTIONS.submit:
        return (
          ...
          >
            ...
            {t("deposit.product_task_form.action.confirm")}
          </Button>
        )
      case TASK_ACTIONS.approve:
        return (
          ...
          >
            <Check className="size-4" />
            {t("deposit.product_task_form.action.approve")}
          </Button>
        )
      case TASK_ACTIONS.requestChanges:
        return (
          ...
          >
            ...
            {t("deposit.product_task_form.action.request_changes")}
          </Button>
        )
      case TASK_ACTIONS.reject:
        return (
          ...
          >
            <X className="size-4" />
            {t("deposit.product_task_form.action.reject")}
          </Button>
        )
      default:
        return null
    ...
    <div className="space-y-4">
      ...
      {!maker && !readOnly ? (
        <div className="space-y-1.5">
          <Label htmlFor="dpm-product-request-comment">
            {t("deposit.product_task_form.comment_label")}
          </Label>
          <Textarea
            id="dpm-product-request-comment"
            rows={3}
            value={comment}
            disabled={submitting}
            placeholder={t("deposit.product_task_form.comment_placeholder")}
            onChange={(event) => {
      ...
      {readOnly ? (
        <p className="text-sm text-muted-foreground">
          {t("deposit.product_task_form.view_only")}
        </p>
      ) : null}

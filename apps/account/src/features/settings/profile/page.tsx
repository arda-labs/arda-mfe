  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
      <section className="min-w-0 rounded-lg border bg-card p-5 md:p-6">
        <div className="flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">{t("profile.description")}</p>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <ReadOnlyField label={t("common.field.name")} value={user?.name || ""} icon={User2} />
          <div className="min-w-0">
            <Label className="text-xs font-medium text-muted-foreground">{t("common.field.email")}</Label>
          ...
          <ReadOnlyField label={t("profile.field.user_id")} value={user?.userId || ""} icon={BadgeCheck} />
          <ReadOnlyField label={t("profile.field.subject")} value={user?.sub || ""} icon={ShieldCheck} />
          <ReadOnlyField label={t("profile.field.avatar_file_id")} value={user?.avatarFileId || ""} copyable />
          <ReadOnlyField label={t("profile.field.avatar_url")} value={user?.picture || ""} copyable />
          <ReadOnlyField label="Cover file ID" value={user?.coverFileId || ""} copyable />
          <ReadOnlyField label="Cover image URL" value={user?.coverImage || ""} copyable />
        </div>
      ...
      <aside className="rounded-lg border bg-card p-5 md:p-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{t("profile.avatar.description")}</p>

        <div className="mt-6 flex flex-col items-center gap-4">
          <div className="relative">
            <Avatar className="size-32">
              <AvatarImage src={preview} alt={user?.name || user?.email || "User"} className="object-cover" />
          ...
          <Badge variant={preview ? "secondary" : "outline"}>
            {preview ? t("profile.avatar.status_uploaded") : t("profile.avatar.status_empty")}
          </Badge>
        </div>
      ...
      <Dialog open={emailOpen} onOpenChange={setEmailOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Use an email address you can access for security notifications.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleChangeEmail} disabled={isSavingEmail}>
              {isSavingEmail ? <Spinner className="mr-2 size-4" /> : null}
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="relative mt-1.5 flex items-center">
        {Icon && <Icon className="absolute left-3 size-4 text-muted-foreground" />}
        <Input
          value={value}
          readOnly
          className={Icon ? "truncate pl-10 pr-10" : "truncate pr-10"}
        />
        {copyable && value ? (
          <button
            type="button"

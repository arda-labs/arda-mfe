      ...
      <Card className="overflow-hidden rounded-2xl border bg-card/50 shadow-sm">
        <CardHeader className="border-b border-muted/50 pb-5">
          <CardTitle className="flex items-center justify-between gap-3 text-xl">
            ...
            {status && (
              <Status
                variant={status.is_enrolled ? "success" : "default"}
                className="rounded-full px-3"
              >
                <StatusIndicator />
                <StatusLabel className="text-xs font-semibold">
                  {status.is_enrolled ? "Enabled" : "Disabled"}
                </StatusLabel>
              </Status>
            )}
          </CardTitle>
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="flex items-start gap-3.5 rounded-2xl border border-primary/10 bg-primary/5 p-5">
            ...
            <div className="space-y-1">
              <p className="text-sm font-semibold text-foreground">
                Trusted browsers protection
              </p>
              <p className="text-xs leading-normal text-muted-foreground">
                After you verify MFA during sign-in, you can choose to trust the
                current browser for 30 days. Trusted browsers may skip MFA until
                that trust expires or the device is removed.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-6 border-t border-muted/50 pt-5">
              <div className="flex items-start gap-2.5 text-sm leading-normal text-muted-foreground">
                ...
                <p>
                  ...
                  Authenticator, 1Password, or any TOTP application.
                </p>
              </div>
              ...
              <div className="group space-y-1.5">
                ...
                <div className="relative flex items-center">
                  <Input
                    ...
                    value={secret.secret}
                    readOnly
                    className="rounded-xl border-muted-foreground/10 bg-muted/20 py-5 pr-11 font-mono text-xs"
                  />
                  <button
                    type="button"
                    onClick={() =>
              ...
              <div className="space-y-4 rounded-2xl border border-muted/50 bg-muted/10 p-5">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  ...
                  <Button
                    ...
                    className="rounded-xl px-6 py-5 font-semibold shadow-sm"
                  >
                    Verify & Activate
                  </Button>
                </div>
          ...
          {step === "done" && backupCodes.length > 0 && (
            <div className="space-y-5 border-t border-muted/50 pt-5">
              ...
              <div className="space-y-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
                ...
                <div className="grid grid-cols-2 gap-2 font-mono text-sm tracking-wider text-emerald-800 dark:text-emerald-200">
                  {backupCodes.map((backupCode) => (
                    <span
                      key={backupCode}
                      className="rounded-lg border border-emerald-500/10 bg-background/50 p-1.5 text-center font-semibold"
                    >
                      {backupCode}
                    </span>
                  ))}
              ...
              <p className="text-xs leading-normal text-muted-foreground">
                Each code can be used once to bypass MFA if you lose your
                authentication device. Store them in a safe place.
              </p>
              <Button
                onClick={() => setStep("idle")}
                className="w-full rounded-xl px-6 py-5 font-semibold sm:w-auto"
              >
                Finish Setup
              </Button>
            </div>

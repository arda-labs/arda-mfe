...
function formatSessionMeta(session: Session) {
  return [session.os, session.browser, session.deviceType]
    .filter(Boolean)
    .join(" - ")
}
...
export function SessionsPage() {
  ...
  const [revokeTarget, setRevokeTarget] = useState<Session | "others" | null>(
    null
  )
    ...
    try {
      ...
      setCurrentSessionId(
        result.currentSessionId ??
          result.sessions.find((session) => session.isCurrent)?.id ??
          null
      )
    } catch (reason) {
      setError(reason)
  ...
  if (error) {
    return (
      <div className="p-4 text-destructive">{translateApiError(error)}</div>
    )
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="flex flex-col gap-4 border-b border-muted/50 pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage and revoke your active login sessions on various devices
          </p>
        </div>
        <Button
          variant="destructive"
      ...
      <div className="grid gap-4">
        ...
        {sessions.map((session) => (
          <Card
            key={session.id}
            className={`group border-muted/40 transition-all duration-300 hover:border-muted-foreground/10 hover:shadow-md ${session.isCurrent ? "border-primary/20 bg-primary/5" : "bg-card/50"}`}
          >
            <CardContent className="flex flex-col justify-between gap-4 p-5 md:flex-row md:items-center md:p-6">
              <div className="flex items-start gap-4">
                <div
                  className={`shrink-0 rounded-2xl p-3 ${session.isCurrent ? "bg-primary/10" : "bg-muted/50"}`}
                >
                  {getDeviceIcon(session.deviceType)}
                </div>
                <div className="space-y-1.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-base leading-none font-semibold text-foreground">
                      {formatSessionName(session)}
                    </span>
                    <Status
                      variant={session.isCurrent ? "success" : "default"}
                    ...
                    {session.isTrusted && (
                      <Status
                        variant="success"
                        className="rounded-full border-emerald-500/20 bg-emerald-500/10 px-2.5 text-emerald-600"
                      >
                  ...
                  <p className="text-xs font-medium text-muted-foreground">
                    {formatSessionMeta(session) || "Browser session"}
                  </p>
              ...
              <div className="flex shrink-0 items-center justify-end">
                {!session.isCurrent ? (
                  <Button
                    ...
                    disabled={revoking}
                    className="rounded-xl border-muted-foreground/20 px-4 py-4 text-xs font-semibold transition-colors hover:border-destructive hover:bg-destructive hover:text-destructive-foreground"
                  >
                    Revoke
                  </Button>
                ) : (
                  <span className="rounded-xl bg-primary/10 px-3 py-1 text-xs font-semibold text-primary/80">
                    Current Session
                  </span>
                )}
              </div>
